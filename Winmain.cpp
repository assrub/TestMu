...
{
	int iFind = ( int)'\\'; //92
	char *lpFound = lpszPath; //"xxx\Main.exe"
	char *lpOld = lpFound;    //"xxx\Main.exe"
	while ( lpFound)
	{
		lpOld = lpFound;
...
{
	DWORD dwHandle;
	/*El valor devuelto es el tamaño, en bytes, de la información de versión del archivo..
	1612
	*/
	...
	//obtenemos la versión con corrimiento de bits
...
/*Acá es donde va toda la entradas por ventana (windows)*/
LONG FAR PASCAL WndProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
    switch (msg)
    {
	case WM_SYSKEYDOWN: //cuando el hwnd detecta la tecla alt o f10
		{
			return 0;
		}
		break;
...
{
    char *windowName = "BAHIMU";

    WNDCLASS wndClass; //Nombre de clase de estilo de ventanas
    HWND hWnd;
	
	
    wndClass.style         = CS_HREDRAW | CS_VREDRAW; //Redibuja toda la ventana (Vertical y Horizontal) si un movimiento o ajuste de tamaño cambia el ancho del área del cliente.
    wndClass.lpfnWndProc   = WndProc;
    wndClass.cbClsExtra    = 0;
    wndClass.cbWndExtra    = 0;
    wndClass.hInstance     = hInstance;
    wndClass.hIcon		   = LoadIcon(hInstance, (LPCTSTR)IDI_ICON1); //ICONO DEL MU
    wndClass.hCursor	   = LoadCursor(NULL, IDC_ARROW); //IMAGEN DEL CURSOR
...
BOOL OpenInitFile()
{
	...
	switch(m_Resolution)
	{
	case 0:WindowWidth =  640; WindowHeight =  480; FontHeight = 12; break;
	case 1:WindowWidth =  800; WindowHeight =  600; FontHeight = 13; break;
	case 2:WindowWidth = 1024; WindowHeight =  768; FontHeight = 14; break;
	case 3:WindowWidth = 1280; WindowHeight = 1024; FontHeight = 15; break;
	case 4:WindowWidth = 1680; WindowHeight = 1050; FontHeight = 16; break;
	case 5:WindowWidth = 1920; WindowHeight = 1080; FontHeight = 17; break;
	}
...
{
	MSG msg; //ubicacion temporal para todos los mensajes

	//Captura una excepción y verifica si esta la configuración full screen habilitada
	leaf::AttachExceptionHandler(ExceptionCallback);
	
	
	/*  ###################### CONSOLA DEBUG VERSION    ###################### */
	char lpszExeVersion[256] = "unknown";

    //obtenemos la dirección raiz donde esta el Main "C:\Users\ruben\OneDrive\Disco D\Source\Source-Cliente\Global Debug\Main.exe"
	char *lpszCommandLine = GetCommandLine();
	char lpszFile[MAX_PATH];
	WORD wVersion[4] = { 0,};

	//                           vacio    /   "C:\Users\ruben\OneDrive\Disco D\Source\Source-Cliente\Global Debug\Main.exe"
	if ( GetFileNameOfFilePath( lpszFile, lpszCommandLine))  
	{                        //main.exe   null   
		if ( GetFileVersion( lpszFile, wVersion))
		{
	...
	/* CAMBIADO POR RUBENS
	switch(WindowWidth) //Tamaño de Font según la Resolucion de pantalla
	{
	...
	*/
	...
	//si no esta configurado en regedit, Idioma o Lenguaje predeterminado para iniciar el servidor 
	...
	//CREAMOS LA ESCENA PRINCIPAL - INICIO
